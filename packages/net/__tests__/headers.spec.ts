...
describe(Headers, () => {
  describe("constructor()", () => {
    ...
    it("can be created given a Headers instance", () => {
      ...
      expect(Object.fromEntries(headers.entries())).toStrictEqual({ accept: "*/*" });
    });
    ...
    it("duplicates values for the same header names with different casing", () => {
      ...
      expect(headers.get("accept-encoding")).toBe("gzip, deflate, br, gzip, deflate, br");
    });
  });
  ...
  describe(".keys()", () => {
    it("returns the iterator with the header keys", () => {
      ...
      expect(keys).toStrictEqual(["accept", "accept-language", "content-type"]);
    });
    ...
    it("sorts returned keys alphabetically", () => {
      ...
      headers.set("X-A", "2");
      headers.set("X-C", "3");
      expect([...headers.keys()]).toStrictEqual(["x-a", "x-b", "x-c"]);
    });
    ...
    it("does not combine set-cookie headers", () => {
      ...
      expect([...headers.keys()]).toStrictEqual(["set-cookie", "set-cookie"]);
    });
  });
  ...
  describe(".values()", () => {
    it("returns the iterator with the header values", () => {
      ...
      expect(values).toStrictEqual(["*/*", "en-US", "application/json"]);
    });
    ...
    it("sorts returned values alphabetically", () => {
      ...
      headers.set("X-A", "2");
      headers.set("X-C", "3");
      expect([...headers.values()]).toStrictEqual(["2", "1", "3"]);
    });
    ...
    it("does not combine set-cookie headers", () => {
      ...
      headers.append("Set-Cookie", "a=1");
      headers.append("Set-Cookie", "b=2");
      expect([...headers.values()]).toStrictEqual(["a=1", "b=2"]);
    });
  });
  ...
  describe(".forEach()", () => {
    it("traverses each header once", () => {
      ...
      expect(headerSet).toStrictEqual(new Set(["accept", "user-agent"]));
    });
