 ...
 * file/net/parser helpers through the global object.
 *
 * `globalThis.mptool` 的聚合命名空间，热更新代码可通过全局对象访问 file/net/parser 工具。
 */
 ...
 *
 * Call this manually (e.g. in `app.js`) when you need hot-reload code to access the file/net
 * helpers through the global object.
 *
 * 需要热更新代码通过全局对象访问 file/net 工具时，请手动调用（例如在 `app.js` 中）。
 */
