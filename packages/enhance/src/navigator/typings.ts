...
export interface NavigatorMethods {
   ...
   *
   * ```html
   * <button catch:tap="$bindRedirect" data-url="/pages/play" data-before="onClickBefore">
   *   click redirect
   * </button>
   * ```
   */
   ...
   *
   * ```html
   * <button catch:tap="$bindSwitch" data-url="/pages/play" data-before="onClickBefore">
   *   click switch
   * </button>
   * ```
   */
   ...
   *
   * ```html
   * <button catch:tap="$bindReLaunch" data-url="/pages/play" data-before="onClickBefore">
   *   click relaunch
   * </button>
   * ```
   */
  ...
  /**
   * 点击代理方法，绑定 `$back` 逻辑 你可以在元素上声明 `data-delta` 作为回退层数
   *
   * - `data-before` 跳转前执行
   * - `data-after` 跳转后执行
   *
