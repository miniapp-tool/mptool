...
> 状态：已定稿（2026-08-06 第三轮：全部决策已确认——方案 A 值模型 / 路线 A 帧式求值器 / 微信优先最低基础库 3.8.2 / 内建能委托就委托 / BigInt 默认开 / 暂不加入 @mptool/all）
>
> 日期：2026-08-06
...
| 严格模式                               | 支持 `"use strict"` 指令并实施**实用子集**：`this` 不装箱、未声明赋值抛 `ReferenceError`、禁八进制字面量、`delete` 标识符抛错、重复参数抛 `SyntaxError`、`with`/`arguments.callee` 禁用 | 完整逐条执行成本高，文档列出未强制项                         |
...
- **Golden 对照测试（核心）**：同一段代码在本解释器与 Node 中分别执行、对比结果（async 代码用 `await` 收敛后再比）。
- 每个特性独立 spec：正常路径 + 边界（TDZ、闭包、`this` 绑定、`new`、原型链、严格模式、`for...of` 中途 break）。
- **async 专项**：直链 await、表达式内 await、`for...of` + await、`try/catch/finally` 包 await、并行 `Promise.all`、async 函数作为宿主回调、`run()` 返回 Promise 的收敛语义。
- 安全测试：死循环触发 `maxSteps`、深递归触发 `maxStack`、白名单阻止宿主访问、`constructor.constructor` 逃逸路径行为记录。
- **跨端差异测试**：微信 vs QQ 差异项（`Array.values`、`Symbol`、`TypedArray`、Promise 时序）在文档与测试中显式标注，避免误用。
- 沿用仓库约定：测试 shuffle-safe、`@mptool/mock` 若用到需先 build；可对照 Neil Fraser `js-interpreter`（Apache-2.0）作语义参考与测试基准。
- 按用户工作流，测试补完后交给独立子代理对照 ES 规范/Node 行为审查。
