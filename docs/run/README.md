...
| `global`   | `false`  | 自动暴露宿主全局对象（`globalThis`）的全部自有属性 |
...
- 通过 `globals` 显式传入：`run("a + 1", { globals: { a } })`
- 或开启 `global: true`，自动暴露宿主全局对象 `globalThis` 的全部自有属性（含手动挂到全局的变量，如 `globalThis.a = a` 后代码中可直接访问 `a`）

注意：`import` 引入的模块变量属于调用方模块的**词法作用域**，解释器与 `new Function` 一样无法自动读取调用方的词法变量（JS 语言限制）。如需在代码中访问，请将变量挂到全局或通过 `globals` 传入。
