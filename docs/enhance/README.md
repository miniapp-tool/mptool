...
我们提供了 `defaultPage` 和 `pages` 两个选项来帮助你传递页面路由。
...
你需要在 `defaultPage` 填入一个页面模式，它表示在 `pages` 缺失或无法解析的情况下，对页面简称和页面路径的对应关系。换而言之，这是一个回退选项。

当你具有特别简单的小程序结构并可以完全用 `defaultPage` 表示时，你无需配置 `pages` 选项。但是当你具有复杂的小程序结构 (如分包) 时你可以通过两种方式配置 `pages` 选项:
...
```nginx
location /hot-reload/ {
    try_files $uri @empty200;  # 存在 → 返回文件内容；不存在 → 走空响应
